...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Header ── */
  header { background: #ffffff; border-bottom: 2px solid var(--border); padding: 14px 28px; display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
  .header-title { display: flex; align-items: center; gap: 14px; }
  .header-logo { height: 36px; display: block; }
  h1 { font-size: 1.22rem; font-weight: 700; letter-spacing: -0.02em; color: #1a1a1a; }
  h1 span { color: var(--accent); }
  .header-meta { display: flex; gap: 10px; flex-wrap: wrap; }
  .hbadge { background: var(--surface2); border: 1px solid var(--border); border-radius: 20px; padding: 4px 12px; font-size: 0.77rem; color: #666; }
  .hbadge b { color: #1a1a1a; }
  ...
  .tab-badge.gold { background: rgba(200,160,0,0.12); color: var(--gold); }
  ...
  .player-item.active { background: rgba(200,160,0,0.1); border-left: 3px solid var(--accent); }
  ...
  .player-avatar { width: 52px; height: 52px; border-radius: 50%; background: linear-gradient(135deg, #aaa 0%, var(--accent) 100%); display: flex; align-items: center; justify-content: center; font-size: 1.3rem; font-weight: 700; flex-shrink: 0; color: #fff; }
  ...
  tr:hover td { background: rgba(200,160,0,0.05); }
  .rank-chip { display: inline-block; font-weight: 700; font-size: 0.83rem; padding: 2px 9px; border-radius: 20px; }
  .rank-top3 { background: rgba(200,160,0,0.18); color: var(--gold); }
  .rank-top9 { background: rgba(200,160,0,0.09); color: var(--gold); }
  .rank-normal { color: var(--text2); }
  .rating-pos { color: var(--green); font-weight: 600; }
  .rating-neg { color: var(--red); }
  .rating-zero { color: var(--text2); }
  .btn-view { background: rgba(200,160,0,0.08); color: var(--accent); border: 1px solid rgba(200,160,0,0.22); border-radius: 6px; padding: 5px 12px; font-size: 0.78rem; font-weight: 600; cursor: pointer; white-space: nowrap; text-decoration: none; transition: background .15s; display: inline-block; }
  .btn-view:hover { background: rgba(200,160,0,0.16); }
  ...
  .type-rank   { background: rgba(200,160,0,0.1);  color: var(--gold);  border: 1px solid rgba(200,160,0,0.25); }
  .type-rating { background: rgba(30,122,62,0.1);   color: var(--green); border: 1px solid rgba(30,122,62,0.25); }
  .type-both   { background: rgba(200,160,0,0.12); color: var(--gold);  border: 1px solid rgba(200,160,0,0.3); }
  ...
  .btn-poster { background: #fff; color: var(--accent); border: 1px solid rgba(200,160,0,0.35); border-radius: 6px; padding: 5px 11px; font-size: 0.77rem; font-weight: 600; cursor: pointer; white-space: nowrap; transition: all .15s; }
  .btn-poster:hover { background: rgba(200,160,0,0.08); border-color: var(--accent); }
  ...
  .upload-area:hover { border-color: var(--accent); background: rgba(200,160,0,0.04); }
...
<div class="panel active" id="panel-achievers">
  <div class="achievers-panel">
    <div class="achievers-header">
      ...
      <div id="ach-save-msg" style="font-size:0.81rem;color:#888;"></div>
      <div class="filter-row">
        ...
        <button class="filter-btn" onclick="saveAchievementsToSheet()" style="padding:5px 16px">💾 Save to Sheet</button>
      </div>
    </div>
  ...
  <div class="achievers-panel">
    <div class="achievers-header" style="flex-wrap:wrap;gap:10px">
      ...
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-left:auto">
        <select id="got-month-sel" onchange="loadGotRating()" style="border:1px solid var(--border);border-radius:7px;padding:5px 10px;font-size:0.83rem;background:#fff;color:var(--text);outline:none;cursor:pointer"></select>
...
function buildAchText(r) {
  if (r.pos_override) return r.pos_override;
...
var MONTH_NAMES = {Jan:'January',Feb:'February',Mar:'March',Apr:'April',May:'May',Jun:'June',
  Jul:'July',Aug:'August',Sep:'September',Oct:'October',Nov:'November',Dec:'December'};
function fmtMonth(period) {
  var p = (period||'').split('-');
  return p.length === 2 ? (MONTH_NAMES[p[1]]||p[1]) + ' ' + p[0] : period;
}

var gotRatingResults = [];

function openGotRatingPoster(idx) {
  var r = gotRatingResults[idx];
  if (!r) return;
  var p = r.player;
  var name = p.cr_name || p.player_name;
  var parts = [];
  if (r.gStd && r.std) parts.push('Classical ' + r.std);
  if (r.gRap && r.rap) parts.push('Rapid ' + r.rap);
  if (r.gBli && r.bli) parts.push('Blitz ' + r.bli);
  var ratingStr    = parts.join(' · ') || 'FIDE Rating';
  var primaryType  = r.gStd ? 'Classical' : (r.gRap ? 'Rapid' : 'Blitz');
  var achieveText  = 'Achieved International ' + primaryType + ' FIDE Rating in ' + fmtMonth(r.period);
  currentRow = { player_name: name, tournament_name: achieveText, pos_override: ratingStr };
  document.getElementById('modal-player').value            = name;
  document.getElementById('modal-achievement').textContent = ratingStr;
  document.getElementById('modal-tournament').value        = achieveText;
...
+'.caissa-red { color:#b83030; }';
  ...
  var sc = [
    ...
    '    var cy2=sy+em/2+ci*(em+ls);',
    '    ctx.save();ctx.translate(cx2,cy2);ctx.rotate(-Math.PI/2);',
    '    ctx.strokeText(chs[ci],0,0);ctx.restore();',
    '  }',
    ...
    '  di(md,-50,-35,200,313);',
    '  di(ch,310,543,260,197);',
    '  di(lg,px+pw-170,py+26,160,48);',
    '  dc(pf,153,575,105);',
    '}'
  ].join('\\n');
...
var GAS_URL = 'https://script.google.com/macros/s/AKfycbwwxj4U-F2ScCcJPoGdutZzE4IrADWs6JQcwHSXxRaFLrAtmFAEXBUKSxfoUhE9CLj4Pg/exec';
...
// ── Load from Sheet → auto-fetch API if sheet empty for this month ────────────
function loadGotFromSheet() {
  var month = selectedMonth();
  if (!GAS_URL) { fetchGotRating(); return; }
  setMsg('Checking sheet for ' + month + '…');
  fetch(GAS_URL + '?action=read_got_rating&month=' + encodeURIComponent(month))
    .then(function(r){ return r.json(); })
    .then(function(d) {
      if (!d.ok) { setMsg('Sheet error: ' + d.error + ' — fetching from API…', '#f7a440'); fetchGotRating(); return; }
      var rows = d.rows || [];
      if (rows.length === 0) {
        // Nothing in sheet for this month — auto-fetch from API and save
        setMsg('No data in sheet for ' + month + ' — fetching from FIDE API…');
        fetchGotRating();
        return;
      }
      var playerMap = {};
      ...
      var results = rows.map(function(row) {
        ...
        return { player:p, period:row[3], std:Number(row[4])||0, rap:Number(row[5])||0, bli:Number(row[6])||0,
          gStd:!!(row[4]>0), gRap:!!(row[5]>0), gBli:!!(row[6]>0) };
      });
      document.getElementById('badge-got').textContent = results.length;
      renderGotRating(results, document.getElementById('got-rating-tbody'));
      setMsg('✓ Loaded ' + results.length + ' records from sheet for ' + month, '#1e7a3e');
    })
    .catch(function(){ setMsg('Sheet unavailable — fetching from FIDE API…'); fetchGotRating(); });
}

// ── Auto-load on tab switch or month change: sheet first, API as fallback ─────
...
// ── Save Achievements to Sheet — batches of 50, shows status ─────────────────
function setAchMsg(txt, color) {
  var el = document.getElementById('ach-save-msg');
  if (el) { el.textContent = txt; el.style.color = color || '#888'; }
}
function saveAchievementsToSheet() {
  if (!GAS_URL) { setAchMsg('GAS_URL not set', '#c0392b'); return; }
  setAchMsg('Saving to sheet…', '#888');
  // Columns: Player Name, FIDE ID, Tournament, Rank, Rating ±, Rated, Date
  var allRows = ACHIEVER_ROWS.map(function(r) {
    return [r.player_name, r.fide_id, r.tournament_name,
            r.rank||'', r.rating_change!==null?r.rating_change:'',
            r.is_rated?'RATED':'', r.date||''];
  });
  var BATCH = 50, totalAdded = 0, batches = Math.ceil(allRows.length / BATCH), done = 0;
  function sendBatch(i) {
    if (i >= allRows.length) {
      setAchMsg('✓ Saved ' + totalAdded + ' new rows to sheet (' + allRows.length + ' total)', '#1e7a3e');
      return;
    }
    var chunk = allRows.slice(i, i + BATCH);
    var url = GAS_URL + '?action=write_achievements&rows=' + encodeURIComponent(JSON.stringify(chunk));
    fetch(url)
      .then(function(r){ return r.json(); })
      .then(function(d){
        if (d.ok) totalAdded += (d.added || 0);
        done++;
        setAchMsg('Saving… batch ' + done + '/' + batches, '#888');
        sendBatch(i + BATCH);
      })
      .catch(function(){ sendBatch(i + BATCH); });
  }
  sendBatch(0);
}
...
function renderGotRating(results, tbody) {
  if (!tbody) return;
  gotRatingResults = results;
  if (results.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:#aaa;padding:28px">No results.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = results.map(function(r, i) {
    ...
    return '<tr>' +
      ...
      '<td><button class="btn-poster" onclick="openGotRatingPoster('+i+')">🖼 Poster</button></td>' +
      '</tr>';
  }).join('');
}
</script>
